...
public class JalaliCalendar extends Calendar {
     ...
     *
     */
    private static final long serialVersionUID = 4488296699098891712L;
    ...
    public JalaliCalendar(TimeZone zone, Locale aLocale) {
        ...
        DateTime yearMonthDate = new DateTime(calendar.get(YEAR), calendar.get(MONTH), calendar.get(DATE));
    ...
    public JalaliCalendar(String jalaliDate) {
        this(Integer.parseInt(jalaliDate.substring(0, 4)), Integer.parseInt(jalaliDate.substring(5, 7))-1, Integer.parseInt(jalaliDate.substring(8, 10)) , 0, 0, 0, 0);
    }
    ...
    JalaliCalendar(int year, int month, int dayOfMonth,
        ...
        DateTime yearMonthDate = jalaliToGregorian(new DateTime(fields[1], fields[2], fields[5]));
    ...
    public static String getCurrentJalaliDate() {
        return JalaliCalendar.gregorianToJalali(new DateTime(Calendar.getInstance().get(Calendar.YEAR), Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH))).toString();
    }

    public static String getCurrentTime() {
        DateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
        return dateFormat.format(getInstance().getTime());
    }

    public static String jalaliToGregorian(String jalaliDate) {
        return jalaliToGregorian(new DateTime(Integer.parseInt(jalaliDate.substring(0, 4)), Integer.parseInt(jalaliDate.substring(5, 7))-1, Integer.parseInt(jalaliDate.substring(8, 10)) )).toString();
    }

    public static Date jalaliToGregorianDate(String jalaliDate) {
        if (!StringUtils.hasLength(jalaliDate))
            return null;
        DateTime yearMonthDate= jalaliToGregorian(new DateTime(Integer.parseInt(jalaliDate.substring(0, 4)), Integer.parseInt(jalaliDate.substring(5, 7))-1, Integer.parseInt(jalaliDate.substring(8, 10)) ));
        Calendar caledar =JalaliCalendar.getInstance();
        caledar.set(Calendar.YEAR, yearMonthDate.getYear());
        caledar.set(Calendar.MONTH, yearMonthDate.getMonth());
        caledar.set(Calendar.DAY_OF_MONTH, yearMonthDate.getDate());
        caledar.set(Calendar.HOUR_OF_DAY, 0);
        caledar.set(Calendar.MINUTE, 0);
        caledar.set(Calendar.SECOND, 0);
        caledar.set(Calendar.MILLISECOND, 0);
        return caledar.getTime();
    }

    public static String gregorianToJalali(String gregorianDate)  {
        return gregorianToJalali(new DateTime(Integer.parseInt(gregorianDate.substring(0, 4)), Integer.parseInt(gregorianDate.substring(5, 7))-1, Integer.parseInt(gregorianDate.substring(8, 10)) )).toString();
    }

    public static DateTime gregorianToJalaliDate(String gregorianDate)  {
        return gregorianToJalali(new DateTime(Integer.parseInt(gregorianDate.substring(0, 4)), Integer.parseInt(gregorianDate.substring(5, 7))-1, Integer.parseInt(gregorianDate.substring(8, 10)) ));
    }

    public static String gregorianDateToJalali(Date gregorianDate)  {
        Calendar caledar =JalaliCalendar.getInstance();
        caledar.setTime(gregorianDate);

        return gregorianToJalali(new DateTime(caledar.get(Calendar.YEAR), caledar.get(Calendar.MONTH),caledar.get(Calendar.DAY_OF_MONTH))).toString();
    }

    public static DateTime gregorianDateToJalaliDate(Date gregorianDate)  {
        if (gregorianDate==null)
            return null;
        Calendar caledar =JalaliCalendar.getInstance();
        caledar.setTime(gregorianDate);

        return gregorianToJalali(new DateTime(caledar.get(Calendar.YEAR), caledar.get(Calendar.MONTH),caledar.get(Calendar.DAY_OF_MONTH),caledar.get(Calendar.HOUR_OF_DAY),caledar.get(Calendar.MINUTE),caledar.get(Calendar.SECOND)));
    }
    ...
    public static DateTime gregorianToJalali(DateTime gregorian) {
        ...
        return new DateTime(jalaliYear, jalaliMonth, jalaliDay,gregorian.getHour(),gregorian.getMinute(),gregorian.getSecond());
    }


    public static DateTime jalaliToGregorian(DateTime jalali) {
        if (jalali.getMonth() > 11 || jalali.getMonth() < -11) {
            throw new IllegalArgumentException("ماه در تاریخ نامعتبر است");
        }
    ...
    public static int weekOfYear(int dayOfYear, int year) {
        switch (dayOfWeek(JalaliCalendar.jalaliToGregorian(new DateTime(year, 0, 1)))) {
            case 2:
                dayOfYear++;
                break;
            case 3:
    ...
    public static int dayOfWeek(DateTime yearMonthDate) {
    ...
    protected void computeTime() {
        ...
        if (!isTimeSet && !isTimeSeted) {
            ...
            DateTime yearMonthDate = jalaliToGregorian(new DateTime(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH)));
        ...
        switch (field) {
            ...
            case DST_OFFSET: {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) &&
                    ...
                    cal = new GregorianCalendar();
                    DateTime yearMonthDate = jalaliToGregorian(new DateTime(internalGet(YEAR), internalGet(MONTH), internalGet(DATE)));
                    cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDate(), internalGet(HOUR_OF_DAY), internalGet(MINUTE),
                            internalGet(SECOND));
                    cal.set(field, value);
                    yearMonthDate = gregorianToJalali(new DateTime(cal.get(YEAR), cal.get(MONTH), cal.get(DATE)));
    ...
    protected void computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            //Day_of_Week
            super.set(DAY_OF_WEEK, dayOfWeek(jalaliToGregorian(new DateTime(fields[1], fields[2], fields[5]))));
            //***
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            DateTime yearMonthDate = jalaliToGregorian(new DateTime(get(YEAR), get(MONTH), get(DATE)));
            ...
            gc.add(field, amount);
            yearMonthDate = gregorianToJalali(new DateTime(gc.get(YEAR), gc.get(MONTH), gc.get(DATE)));
            super.set(YEAR, yearMonthDate.getYear());
            super.set(MONTH, yearMonthDate.getMonth());
            super.set(DATE, yearMonthDate.getDate());
    ...
    public DateTime addDate(int field, int amount) {
        add( field,  amount);
        DateTime ymd= new DateTime(get(Calendar.YEAR), get(Calendar.MONTH), get(Calendar.DAY_OF_MONTH));
        return ymd;
    }

    public DateTime rollDate(int field, int amount) {
        roll( field,  amount);
        DateTime ymd= new DateTime(get(Calendar.YEAR), get(Calendar.MONTH), get(Calendar.DAY_OF_MONTH));
        return ymd;
    }
    ...
    public static class DateTime {
        ...
        public String toString() {
            if (hour==-1 && minute==-1 && second==-1)
                return String.format("%04d/%02d/%02d",getYear() ,getMonth() +1,getDate());
            return String.format("%04d/%02d/%02d %02d:%02d:%02d",getYear() ,getMonth() +1,getDate(),getHour(),getMinute(),getSecond());
        }
    }
}